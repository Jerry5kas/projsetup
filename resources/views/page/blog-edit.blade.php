    ...
    <div class="inset-0 flex items-center justify-center h-screen border shadow-md bg-black/80 font-lex">
        <div class="space-y-5 text-center ">
            ...
            <form action="/blog-edit/{{ $post->id }}" method="POST"
                ...
                <textarea name="body" placeholder="Blog Description ..." class="px-1 py-2 text-xs border rounded">{{ $post->body }}</textarea>
                <button class="px-4 py-1 mx-auto text-white bg-teal-600 rounded max-w-max">Save Changes</button>
            </form>
        </div>
    </div>
</x-layouts.guest>
